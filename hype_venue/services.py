...
def calc_venue_hype_score(venue, all_venue_fb_stat_by_venue_id, all_venue_twitter_stat_by_venue_id):
    ...
    if fb_stat:
        fb_score = ((fb_stat.likes * params.data['fb_likes_weight']) +
                    (fb_stat.checkins * params.data['fb_checkins_weight'])) * params.data['fb_overall_weight']
    else:
        fb_score = 0

    if twitter_stat:
        twitter_score = (twitter_stat.followers_count * params.data['twitter_followers_weight']) * \
                        params.data['twitter_overall_weight']
    else:
        twitter_score = 0
